...
这是一个标准的 **Agent Skill**：一个含 `SKILL.md` 的目录。安装 = 把这个目录（或其符号链接）放进 Agent 的 skills 目录，**目录名用 `codex-automation-cleanup`**。
...
Codex 用 ~/.codex/skills），目录名用 codex-automation-cleanup，可以用软链接。装完确认 SKILL.md 在位。
```
...
# 不想用软链就直接拷贝： cp -R "$SKILL_SRC" ~/.claude/skills/codex-automation-cleanup
```
...
重开（或刷新）Claude Code 后，`SKILL.md` 的 `description` 命中时会自动加载；也可直接说「用 codex-automation-cleanup 收拾一下这个目录」。
...
# 或 cp -R "$SKILL_SRC" ~/.codex/skills/codex-automation-cleanup
```
...
```
<agent 配置根>/skills/codex-automation-cleanup/
```
