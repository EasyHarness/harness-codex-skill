...
Single source of truth: ``requirement`` units do NOT get their own file. They
are merged into automation.toml's ``prompt`` by restructure_toml.py (run before
this script). This script only handles the leftovers:
  ...
  - backs up each touched source file into a trash dir and rewrites it to a
    short stub pointing at the new structure (toml for rules, run-history for logs).
...
def main():
    ...
    print("=== 分流计划 ===")
    print(f"requirement: {counts.get('requirement', 0)}（已并入 automation.toml 的 prompt） "
    ...
    for s in sorted(touched_sources):
        ...
        sp.write_text(_stub(sp.name, args.history_out, trash, ts), encoding="utf-8")
    print(f"\n已写出 {args.history_out}；源文件已备份至 {trash} 并重写为指针。")
...
def _stub(name, hist_out, trash, ts):
    return (
        f"# {name}（已整理 {ts}）\n\n"
        f"本文件已被 harness-codex-skill 整理，以重建 single source of truth：\n\n"
        f"- **用户要求/规则** 已合并进 automation.toml 的 `prompt`（唯一真源）。\n"
        f"- **运行/过程记录** 已归档到 `{hist_out}`。\n"
        f"- 原始内容备份：`{trash}/{name}`。\n\n"
        f"> 请不要再把长期要求写到本文件；规则与要求应进 automation.toml，运行记录应进 {hist_out}。\n"
    )
